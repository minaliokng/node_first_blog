...
router.put('/:comId', async (req, res) => {
    ...
    try{
        const thisCom = await Comments.findOne({"_id": comId});

        if(!thisCom) return res.status(400).json({message: '댓글 조회에 실패하였습니다.'});
        if(!content || content.trim() === '') return res.status(400).json({message: '댓글 내용을 입력해주세요.'});
    
        if(thisCom["password"] !== password) return res.status(400).json({message: '비밀번호가 일치하지 않습니다.'});
